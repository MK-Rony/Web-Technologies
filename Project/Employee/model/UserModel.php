...
class UserModel {
    ...
    public function addOrder($orderedDate, $orderId, $orderStatus) {
        $stmt = $this->conn->prepare("INSERT INTO orderss (order_id, ordered_date, order_status) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $orderedDate, $orderId, $orderStatus);
        return $stmt->execute();
    }

    public function deleteOrder($orderId) {
        $stmt = $this->conn->prepare("DELETE FROM orderss WHERE order_id = ?");
        $stmt->bind_param("s", $orderId);
        return $stmt->execute();
    }

    public function addProduct($productId, $productName, $productStatus, $productPrice) {
        $stmt = $this->conn->prepare("INSERT INTO productss (product_id, product_name, product_status, product_price) VALUES (?, ?, ?, ?)");
    ...
    public function deleteProduct($productId) {
        $stmt = $this->conn->prepare("DELETE FROM productss WHERE product_id = ?");
